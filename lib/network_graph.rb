...
#
# By default it uses a relationship limit of 10, but you can pass
# in a different limit or use nil for unlimited
...
class NetworkGraph
  ...
  def relationships(id)
    Relationship.includes(:entity, :related)
                .where('is_deleted = 0 AND (entity1_id = ? OR entity2_id = ?)', id, id)
  end
  ...
  def entity_from_relationship(rel, who_not_to_select = @entity)
    rel.entity == who_not_to_select ? rel.related : rel.entity
  end
